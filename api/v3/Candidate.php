...
function civicrm_api3_candidate_get ($params) {
  ...
  if (array_key_exists ("group",$params)) {
    ...
    $join .= "join (select max(id) as hist_id, contact_id,date,status from civicrm_subscription_history where group_id=%1 group by contact_id) hist on hist.contact_id=c.id and hist.status='Added'";
